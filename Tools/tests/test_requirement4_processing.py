...
def test_process_requirement4_preserves_neuen(tmp_path: Path) -> None:
    input_path = tmp_path / "in.md"
    output_path = tmp_path / "out.md"
    input_path.write_text(
        "\n".join(
            [
                ...
                "%VOCAB (German) ver 3",
                "word: neuen Vertrag",
                "meaning: neuen Vertrag = aktuell / current",
                "de_1: Wir prüfen den neuen Vertrag.",
                "en_1: We review the new contract.",
                "Tags: phrase",
                "EEND",
                "",
            ]
    ...
    out = output_path.read_text(encoding="utf-8")
    assert "word: neuen Vertrag\n" in out
    assert "meaning: neuen Vertrag = aktuell / current\n" in out
    assert "de_1: Wir prüfen den neuen Vertrag.\n" in out
